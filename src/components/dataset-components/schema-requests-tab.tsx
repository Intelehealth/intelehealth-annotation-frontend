...
import {
  schemaRequestsAPI,
  SchemaChangeRequestResponse,
} from "@/lib/api/schema-requests";
...
export function SchemaRequestsTab({
  datasetId,
  onNavigateToOverview,
}: SchemaRequestsTabProps) {
  ...
  const renderDiff = (req: SchemaChangeRequestResponse) => {
    switch (req.type) {
        ...
        return (
          <div className="bg-green-50/50 border border-green-150 rounded-lg p-3 space-y-2 text-xs">
            <div className="flex items-center gap-1.5 font-bold text-green-700">
              <span className="bg-green-150 text-green-800 text-[10px] px-1.5 py-0.5 rounded">
                PROPOSE ADD FIELD
              </span>
            ...
            <div className="grid grid-cols-2 gap-2 text-slate-600 mt-1">
              <div>
                ...
                {req.field?.questionTitle || req.field?.fieldName}
              </div>
              <div>
                <strong>Type:</strong>{" "}
                {req.field?.columnType || req.field?.fieldType}
              </div>
              <div>
                <strong>Required:</strong>{" "}
                {req.field?.isRequired ? "Yes" : "No"}
              </div>
              <div>
                <strong>Description:</strong>{" "}
                {req.field?.questionDescription || "None"}
              </div>
            </div>
            {req.field?.options && req.field.options.length > 0 && (
              <div className="mt-1">
                <strong>Options:</strong>{" "}
                <code className="bg-white px-1.5 py-0.5 border rounded font-mono">
                  {JSON.stringify(req.field.options)}
                </code>
              </div>
            )}
          </div>
        ...
        return (
          <div className="bg-purple-50/50 border border-purple-150 rounded-lg p-3 space-y-2 text-xs text-left">
            <div className="flex items-center gap-1.5 font-bold text-purple-700 border-b border-purple-100 pb-1.5 mb-2">
              <span className="bg-purple-150 text-purple-800 text-[10px] px-1.5 py-0.5 rounded">
                PROPOSE ADD GROUP
              </span>
        ...
        return (
          <div className="bg-red-50/50 border border-red-150 rounded-lg p-3 space-y-1 text-xs text-left">
            <div className="flex items-center gap-1.5 font-bold text-red-700">
              <span className="bg-red-150 text-red-800 text-[10px] px-1.5 py-0.5 rounded">
                PROPOSE DELETE FIELD
              </span>
              <span className="font-mono">{req.fieldName}</span>
            </div>
            <p className="text-slate-500 text-[11px] mt-1">
              Request to permanently remove this question from the workbench
              config.
            </p>
          </div>
        );
        ...
        return (
          <div className="bg-blue-50/50 border border-blue-150 rounded-lg p-3 space-y-2 text-xs text-left">
            <div className="flex items-center gap-1.5 font-bold text-blue-700">
              <span className="bg-blue-150 text-blue-800 text-[10px] px-1.5 py-0.5 rounded">
                PROPOSE RENAME
              </span>
              <span className="font-mono">{req.fieldName}</span>
            </div>
      ...
      case "UPDATE_FIELD":
        const isGroupUpdate =
          req.field && (req.field.groupId || req.field.fields);
        if (isGroupUpdate) {
          return (
            <div className="bg-purple-50/50 border border-purple-150 rounded-lg p-3 space-y-2 text-xs text-left">
              <div className="flex items-center gap-1.5 font-bold text-purple-700 border-b border-purple-100 pb-1.5 mb-2">
                <span className="bg-purple-150 text-purple-800 text-[10px] px-1.5 py-0.5 rounded">
                  PROPOSE UPDATE GROUP
                </span>
                <span>{req.field?.groupTitle || req.field?.groupName}</span>
              </div>
              <RenderGroupTree group={req.field} />
        ...
        return (
          <div className="bg-amber-50/50 border border-amber-150 rounded-lg p-3 space-y-2 text-xs text-left">
            <div className="flex items-center gap-1.5 font-bold text-amber-700">
              <span className="bg-amber-150 text-amber-800 text-[10px] px-1.5 py-0.5 rounded">
                PROPOSE CHANGE PROPERTIES
              </span>
              <span className="font-mono">{req.fieldName}</span>
            </div>
            <div className="grid grid-cols-2 gap-2 text-slate-600 mt-1">
      ...
      default:
        return (
          <pre className="text-[10px] p-2 bg-slate-100 rounded">
            {JSON.stringify(req.field, null, 2)}
          </pre>
        );
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        <Button
          ...
          className="text-slate-600 hover:bg-slate-100 h-8"
        >
      ...
      <Card className="border-slate-150 shadow-sm">
        <CardHeader className="pb-3 border-b border-slate-100">
          ...
          <CardDescription>
            Review and approve modifications requested by annotators. Merging
            changes updates parent configuration and synchronizes clone
            workspaces.
          </CardDescription>
        </CardHeader>
          ...
          {requests.length === 0 ? (
            <div className="text-center py-16 text-slate-500">
              ...
              <h4 className="text-sm font-semibold text-slate-700 mb-0.5">
                No Pending Schema Requests
              </h4>
              <p className="text-xs text-slate-400">
                All clones and parent configuration are fully in sync.
              </p>
            </div>
          ) : (
              ...
              {requests.map((req) => {
                ...
                const annotatorEmail =
                  createdBy && typeof createdBy !== "string"
                    ...
                    : "";
                const cloneName =
                  typeof req.cloneId === "object" && req.cloneId !== null
                    ? req.cloneId.name
                    : "Clone Workspace";
                ...
                return (
                  <div
                    key={req._id}
                    className="p-5 flex flex-col md:flex-row gap-5 items-start"
                  >
                    {/* Metatada Details */}
                    <div className="w-full md:w-1/4 space-y-2 text-xs text-slate-600">
                      <div>
                        ...
                        <strong className="text-slate-800 text-[13px]">
                          {annotatorName}
                        </strong>
                        <span className="block text-slate-400 text-[11px] font-mono mt-0.5">
                          {annotatorEmail}
                        </span>
                      </div>
                      <div>
  ...
  return (
    <div className="font-mono text-xs leading-relaxed space-y-1 mt-2">
      ...
      {group.fields && group.fields.length > 0 ? (
        ...
        <div className="ml-4 text-xs text-red-500 italic">
          No fields configured inside this repeat group.
        </div>
      )}
    </div>
  );
