...
export function RecursiveFieldEditor({
  field,
  depth,
  onChange,
  onDelete,
    ...
    ? effectiveType === "rating"
      ? Array.from({ length: field.maxRating || 5 }, (_, i) => {
          ...
          const existing = field.branching?.options?.find(
            (o) =>
              String(o.value).trim().toLowerCase() === val.trim().toLowerCase(),
          );
          return (
            existing || {
              value: val,
      ...
      : effectiveType === "checkbox"
        ...
        : field.options && field.options.length > 0
          ? field.options.map((optStr) => {
              ...
              const baseVal =
                colonIdx === -1 ? optStr : optStr.slice(0, colonIdx);
              const existing = field.branching?.options?.find(
                (o) =>
                  String(o.value).trim().toLowerCase() ===
                  baseVal.trim().toLowerCase(),
              );
              return (
                existing || {
                  value: baseVal,
  ...
  const hasBranching = options.some(
    (o) => (o.childFields?.length || 0) > 0 || o.requireDescription,
  );
  ...
  const addChild = (optIdx: number) => {
    ...
    next[optIdx] = {
      ...next[optIdx],
      childFields: [...(next[optIdx].childFields || []), child],
    };
    updateAll(next);
  };
          ...
          {effectiveType === "rating" && (
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  onChange({ ...field, maxRating: Number(e.target.value) })
                }
        ...
        <div className={cn("space-y-2", depth > 0 && "mt-2")}>
          <div className="flex items-center justify-between px-1">
            <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
              {effectiveType === "rating"
                ? "Rating Options & Nested Questions"
                : "Decision Options & Nested Questions"}
            </div>
            {effectiveType !== "rating" && (
          ...
          {options.length > 0 ? (
            options.map((opt, optIdx) => (
              <OptionBranch
                ...
                onUpdateChild={(childIdx, updated) =>
                  updateChild(optIdx, childIdx, updated)
                }
          ...
          {isRating ? (
            ...
            <>
              <Input
                value={option.value}
                onChange={(e) =>
                  onUpdate({ value: e.target.value.replace(/\./g, "") })
                } // Phase 3: sanitize dots
              ...
              <Input
                ...
                onChange={(e) =>
                  onUpdate({ descriptionPlaceholder: e.target.value })
                }
