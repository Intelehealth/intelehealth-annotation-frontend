...
const layers = [
  {
    ...
    icon: Monitor,
    gradient:
      "from-blue-500/10 to-blue-50/30 dark:from-blue-900/50 dark:to-indigo-900/50",
  ...
  },
  {
    ...
    icon: Shield,
    gradient:
      "from-purple-500/10 to-purple-50/30 dark:from-purple-900/50 dark:to-purple-900/50",
  ...
  },
  {
    ...
    icon: Cpu,
    gradient:
      "from-amber-500/10 to-amber-50/30 dark:from-amber-900/50 dark:to-amber-900/50",
  ...
  },
  {
    ...
    icon: Database,
    gradient:
      "from-green-500/10 to-green-50/30 dark:from-green-900/50 dark:to-green-900/50",
  ...
  },
  {
    ...
    icon: HardDrive,
    gradient:
      "from-gray-500/10 to-gray-50/30 dark:from-gray-800/50 dark:to-gray-800/50",
    pill: "bg-gray-100 text-gray-700 dark:bg-gray-900/50 dark:text-gray-300",
...
const dbCollections = [
  {
    ...
    purpose:
      "Stores user accounts, credentials, roles, and profile information",
    icon: Users,
  },
  {
    name: "Datasets",
    color: "purple" as BadgeColor,
    purpose: "Dataset metadata, source CSV references, and lifecycle state",
    icon: Database,
  },
  {
    ...
    purpose:
      "Configured schema fields, validation rules, and lock state per dataset",
    icon: FileText,
  },
  {
    name: "UploadHistory",
    color: "amber" as BadgeColor,
    purpose:
      "CSV upload events with file name, row count, and status per dataset",
    icon: Upload,
  },
  {
    name: "CloneHistory",
    ...
    purpose:
      "Clone creation records linking parent datasets to annotator copies",
    icon: GitBranch,
  },
  {
    name: "DatasetAssignments",
    color: "green" as BadgeColor,
    purpose:
      "Annotator-to-clone mapping for access control and progress tracking",
    icon: List,
  },
  {
    name: "MergedRows",
    color: "indigo" as BadgeColor,
    purpose: "Consolidated annotation values after clone reconciliation",
    icon: Edit3,
  },
  {
    ...
    purpose:
      "Final, authoritative record values with resolution method metadata",
    icon: CheckCircle2,
  },
  {
    name: "ConsensusReview",
    color: "emerald" as BadgeColor,
    purpose:
      "Per-row consensus calculations, field reviews, annotator comparisons, branch mismatch flags, and admin override decisions",
    icon: CheckCircle2,
  },
  {
    name: "Notifications",
    color: "rose" as BadgeColor,
    purpose: "In-app notification queue for assignment and consensus events",
    icon: Bell,
  },
];
...
const apiOutcomes = [
  {
    label: "200 Success",
    desc: "Request processed and data returned",
    color:
      "bg-green-50 text-green-700 border-green-200/60 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800/60",
  },
  {
    label: "400 Bad Request",
    desc: "Malformed payload or validation failure",
    color:
      "bg-orange-50 text-orange-700 border-orange-200/60 dark:bg-orange-900/30 dark:text-orange-300 dark:border-orange-800/60",
  },
  {
    label: "401 Unauthorized",
    desc: "Missing or invalid JWT token",
    color:
      "bg-red-50 text-red-700 border-red-200/60 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800/60",
  },
  {
    label: "403 Forbidden",
    desc: "Valid token but insufficient role permissions",
    color:
      "bg-red-50 text-red-700 border-red-200/60 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800/60",
  },
];
...
const timelineSteps = [
  {
    step: 1,
    actor: "Admin",
    label: "Uploads CSV dataset",
    color: "bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300",
  },
  {
    step: 2,
    actor: "System",
    label: "Validates and ingests records",
    color:
      "bg-purple-100 text-purple-700 dark:bg-purple-900/50 dark:text-purple-300",
  },
  {
    step: 3,
    actor: "Admin",
    label: "Configures annotation schema",
    color: "bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300",
  },
  {
    step: 4,
    actor: "Admin",
    label: "Clones dataset and assigns annotators",
    color: "bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300",
  },
  {
    step: 5,
    actor: "Annotator A",
    label: "Annotates assigned records independently",
    color:
      "bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-300",
  },
  {
  ...
  },
  {
    ...
    actor: "System",
    label:
      "Schema Synchronization — auto-increments version, syncs clones, preserves compatible answers, marks PENDING_UPDATE",
    color:
      "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300",
  },
  {
    ...
    actor: "System",
    label:
      "Detects agreement, generates skeleton reviews (zero-blocking), flags conflicts, assigns field status lifecycle",
    color:
      "bg-purple-100 text-purple-700 dark:bg-purple-900/50 dark:text-purple-300",
  },
  {
    ...
    actor: "Admin",
    label:
      "Opens Generate Consensus (always available) and resolves conflicts using dynamic type-aware widgets",
    color: "bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300",
  },
  {
    step: 10,
    actor: "Admin",
    label:
      "Reviews hierarchical comparison matrix in Review Consensus — all rows and fields visible even if unstarted",
    color: "bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300",
  },
  {
    step: 11,
    actor: "System",
    label:
      "Generates final consensus dataset with current schema, including admin decisions and status",
    color:
      "bg-purple-100 text-purple-700 dark:bg-purple-900/50 dark:text-purple-300",
  },
];
...
const tiers = [
  {
    label: "Core",
    color: "blue" as BadgeColor,
    chips: ["Authentication", "User Management", "Role-Based Access"],
  },
  {
    label: "Workflow",
    color: "purple" as BadgeColor,
    chips: ["Dataset Management", "Field Configuration", "Dataset Cloning"],
  },
  {
    label: "Quality",
    color: "amber" as BadgeColor,
    chips: ["Annotation Workbench", "Consensus Engine", "Review Workspace"],
  },
  {
    label: "Support",
    color: "green" as BadgeColor,
    chips: ["Notifications", "Dashboards", "Data Export"],
  },
];
...
const moduleDetails = [
  {
    name: "Authentication",
    role: "Verifies user identity and enforces role-based access",
    actions: ["JWT login", "Google OAuth", "Session management"],
    color: "blue" as BadgeColor,
  },
  {
    name: "Dataset Management",
    role: "Governs CSV ingestion and dataset lifecycle",
    actions: ["CSV upload", "Duplicate detection", "Row ID assignment"],
    color: "purple" as BadgeColor,
  },
  {
    name: "Field Configuration",
    role: "Defines annotation schema and validation rules",
    actions: ["Field types", "Required fields", "Schema lock"],
    color: "amber" as BadgeColor,
  },
  {
    name: "Dataset Cloning",
    role: "Creates isolated copies for each annotator",
    actions: ["Clone creation", "Annotator assignment", "Row propagation"],
    color: "green" as BadgeColor,
  },
  {
    name: "Annotation Workbench",
    role: "Primary interface for data labeling",
    actions: ["Autosave", "Keyboard nav", "Progress tracking"],
    color: "indigo" as BadgeColor,
  },
  {
    name: "Consensus Engine",
    role: "Detects agreement, generates skeleton reviews, and resolves conflicts with zero-blocking access",
    actions: [
      "Auto-accept",
      "Conflict flagging",
      "Admin override",
  ...
  {
    name: "Generate Consensus",
    role: "Interactive resolution console with type-aware dynamic value pickers and skeleton review support",
    actions: [
      "Zero-blocking open",
      "Skeleton review",
      "Field override",
  ...
  },
  {
    ...
    role: "Read-only hierarchical comparison matrix matching Annotation Workbench tree exactly",
    actions: [
      "Tree hierarchy",
      "NOT_STARTED display",
      "PENDING_UPDATE",
  ...
  },
  {
    ...
    role: "Auto-increments version, syncs clones, preserves compatible answers, marks PENDING_UPDATE on schema changes",
    actions: [
      "Version increment",
      "Clone sync",
      "Answer preservation",
  ...
  },
  {
    ...
    role: "Manages accounts and access permissions",
    actions: ["Invitations", "Status lifecycle", "Search & filter"],
    color: "cyan" as BadgeColor,
  },
  {
    name: "Notifications",
    role: "Alerts users to assignment and consensus events",
    actions: ["Clone alerts", "Consensus updates", "Dashboard feeds"],
    color: "rose" as BadgeColor,
  },
  {
    name: "Data Export",
    role: "Produces final, model-ready CSV output",
    actions: ["CSV generation", "Field inclusion", "Audit trail"],
    color: "sky" as BadgeColor,
  },
];
...
const adminScreens = [
  {
    name: "Dashboard",
    desc: "Consolidated platform metrics overview",
    icon: LayoutDashboard,
  },
  {
    name: "Datasets",
    desc: "List with search, filter, and CRUD",
    icon: Database,
  },
  {
    name: "Dataset Detail",
    desc: "Upload history, field config, clones",
    icon: FileText,
  },
  {
    name: "Clone Assignment",
    desc: "Clone creation and annotator assignment",
    icon: GitBranch,
  },
  {
    name: "Generate Consensus",
    desc: "Skeleton review generation with dynamic override widgets (never blocks)",
    icon: Wand2,
  },
  {
    name: "Review Consensus",
    desc: "Hierarchical tree comparison matrix matching workbench exactly",
    icon: ScanEye,
  },
  {
    name: "User Management",
    desc: "Search, invite, enable, disable, delete",
    icon: Users,
  },
  {
    name: "User Invitation",
    desc: "Bulk invite form for new users",
    icon: UserCheck,
  },
  {
    name: "Settings",
    desc: "Profile and account configuration",
    icon: BookOpen,
  },
];
...
const annotatorScreens = [
  {
    name: "Dashboard",
    desc: "Assigned tasks and progress summary",
    icon: LayoutDashboard,
  },
  {
    name: "Annotation Workbench",
    desc: "Record-by-record labeling interface",
    icon: NotebookPen,
  },
  {
    name: "Settings",
...
const dataFlowStages = [
  ...
  },
  {
    ...
    icon: Download,
    color:
      "bg-teal-50 dark:bg-teal-900/40 border-teal-200 dark:border-teal-800 text-teal-700 dark:text-teal-300",
  },
];
...
const lifecycleStates = [
  {
    ...
    transition: "Invite sent",
  },
  {
    state: "Active",
    color:
      "bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300 border-green-300 dark:border-green-700",
    transition: "First login",
  },
  {
    state: "Inactive",
    color:
      "bg-gray-100 text-gray-700 dark:bg-gray-900/50 dark:text-gray-300 border-gray-300 dark:border-gray-700",
    transition: "Logout / timeout",
  },
  {
    state: "Disabled",
    color:
...
const docPages = [
  {
    ...
    purpose:
      "Quick onboarding guide for new users to access and navigate the platform.",
  ...
  },
  {
    ...
    purpose:
      "Provides a high-level understanding of the architecture, workflow phases, and business goals.",
  ...
  },
  {
    ...
    purpose:
      "Explains how administrators initialize and name new datasets in the system.",
  ...
  },
  {
    ...
    purpose:
      "Guides the administrator on defining the annotation targets and validation rules.",
    reader: "Administrators",
    features: [
      "Visual Schema Builder",
      "Field Validation Toggles",
      "Dynamic Layout Customizer",
      "Automatic Schema Version Increment",
      "Clone Synchronization Engine",
    ],
    steps: [
  ...
  },
  {
    ...
    purpose:
      "Distributing dataset copies (clones) to annotators for independent labeling.",
    reader: "Administrators",
    features: [
      "Annotator Assignment Checklist",
      "Physical Cloning Trigger",
      "Progress Metric Tracing",
    ],
    steps: [
  ...
  },
  {
    ...
    purpose:
      "Primary workspace where annotators enter label values for assigned datasets.",
    reader: "Annotators",
    features: [
  ...
  },
  {
    ...
    purpose:
      "Tabular read-only grid displaying comparison results across all annotator workbenches, matching the workbench tree hierarchy exactly. Never empty — always renders all rows and fields.",
    reader: "Administrators, Project Managers",
    features: [
      "Hierarchical Row Matrix",
      "Unstarted/Pending Annotator Flags",
      "Field Status Indicator Lifecycle (NOT_STARTED → PENDING → PARTIAL → AGREED/CONFLICT → ADMIN_CONFIRMED/OVERRIDDEN)",
      "Original CSV Side-by-Side Display",
      "Zero-Blocking Skeleton Generation",
      "PENDING_UPDATE Schema Sync Detection",
    ],
    steps: [
  ...
  },
  {
    ...
    purpose:
      "Action-oriented workflow allowing administrators to generate review skeletons and override annotator conflicts. Always available — never blocks access.",
    reader: "Administrators",
    features: [
      "Zero-Blocking Skeleton Review Generator",
      "Conflict Resolution Panel with Quick-Action Buttons",
      "Schema Synchronization Engine",
      "Dynamic Type-Aware Value Override Widget (Rating, Range, Multi-select, Select, Boolean, Textarea, Text)",
    ],
    steps: [
  ...
  },
  {
    ...
    purpose:
      "Automatic propagation of schema changes across all annotator clones and consensus pages.",
    reader: "Administrators",
    features: [
      "Schema Version Increment",
      "Automatic Clone Synchronization",
      "Compatible Answer Preservation",
      "PENDING_UPDATE Marking for Incompatible Fields",
      "Field Archiving (Soft Delete)",
      "Cross-Module Update (Workbench, Tasks, Generate Consensus, Review Consensus, Exports, Statistics)",
    ],
    steps: [
  ...
  },
  {
    title: "Generate Consensus",
    purpose:
      "Action-oriented workflow allowing administrators to generate review skeletons and override annotator conflicts.",
    reader: "Administrators",
    features: [
      "Skeleton Review Generator",
      "Conflict Resolution Panel",
      "Schema Synchronization Engine",
      "Dynamic Value Override Widget",
    ],
    steps: [
  ...
  },
  {
    ...
    purpose:
      "Enables administrators to invite, query, search, disable, and delete users.",
    reader: "Administrators",
    features: [
      "User Search and In-place Filter",
  ...
  },
  {
    title: "Notifications",
    purpose:
      "Delivers real-time assignment notifications and consensus alerts to users.",
    reader: "All Roles",
    features: [
      "Real-time Header Bell Icon",
      "In-App Alert Notification Queue",
  ...
  },
  {
    title: "Dashboard",
    purpose:
      "Aggregated analytics and tracking hub tailored to the active user's role.",
    reader: "All Roles",
    features: [
      "KPI Status Metric Cards",
      "Live In-Progress Grid",
  ...
  },
  {
    ...
    purpose:
      "Managing individual profile details, passwords, and user preferences.",
    reader: "All Roles",
    features: [
      "Name and Details Editor",
  ...
  },
  {
    ...
    purpose:
      "Details the Hotkeys configured to enable high-speed data labeling.",
    reader: "Annotators",
    features: [
      "Workspace Overlay Guide",
      "Navigation Hotkeys",
      "Input Hotkeys",
    ],
    steps: [
  ...
  },
  {
    ...
    purpose:
      "A collection of standard questions and answers regarding platform capabilities.",
    reader: "All Roles",
    features: [
      "FAQ Topic Categories",
      "Search & Expand Accordion",
      "Feedback Form Link",
    ],
    steps: [
  ...
  },
  {
    ...
    purpose:
      "Practical solutions to solve common UI, network, and application issues.",
    reader: "All Roles",
    features: [
  ...
  },
  {
    ...
    purpose:
      "Interactive OpenAPI sandbox environment describing all backend endpoints.",
    reader: "Developers, System Administrators",
    features: [
      "OpenAPI Swagger UI Sandbox",
      "Dynamic Endpoint Tester",
      "JSON Schema Models List",
    ],
    steps: [
  ...
  },
  {
    ...
    purpose:
      "Explains how to compile, configure, deploy, and monitor the containerized platform.",
    reader: "DevOps Engineers, System Administrators",
    features: [
      "Docker Compose Configuration",
      "Environment Schema Guide",
...
const configCategories = [
  {
    ...
    items: [
      {
        name: "Label",
        desc: "Displays the field name. Keep it short and descriptive so annotators immediately understand the field's purpose.",
        types: ["All Types"],
        mockType: "label",
      },
      {
        name: "Description",
        desc: "Provides secondary explanation text directly under the label, offering definition details.",
        types: ["All Types"],
        mockType: "desc",
      },
      {
        name: "Placeholder",
        desc: 'Displays faded inline hint text inside empty text or numeric inputs (e.g. "Enter name...").',
        types: ["Text", "Number"],
        mockType: "placeholder",
      },
      {
        name: "Default Value",
        desc: "Pre-fills the input with a predefined value when a record is first opened, saving typing effort.",
        types: ["Text", "Number", "Dropdown"],
        mockType: "default",
      },
      {
        name: "Display Order",
        desc: "An integer used to arrange fields from top to bottom. Lower numbers render first.",
        types: ["All Types"],
        mockType: "order",
      },
      {
        name: "Field Width",
        desc: "Controls grid layout allocation (e.g. half-width columns for side-by-side inputs, full-width for textareas).",
        types: ["All Types"],
        mockType: "width",
      },
      {
        name: "Read Only",
        desc: "Prevents annotator input on a target field while keeping the value visible for context.",
        types: ["All Types"],
        mockType: "readonly",
      },
    ],
  },
  {
    ...
    items: [
      {
        name: "Required",
        desc: "Blocks record saving and marks the record as incomplete if the target field is left empty or unselected.",
        types: ["All Types"],
        mockType: "required",
      },
      {
        name: "Minimum / Maximum Length",
        desc: "Restricts text inputs to a specific range of characters, avoiding empty inputs or excessively long essays.",
        types: ["Text"],
        mockType: "length",
      },
      {
        name: "Minimum / Maximum Value",
        desc: "Restricts numeric inputs mathematically (e.g. age must be between 0 and 120).",
        types: ["Number"],
        mockType: "value",
      },
      {
        name: "Regex Validation",
        desc: "Validates inputs against standard regular expressions (e.g. email patterns, phone number strings, codes).",
        types: ["Text"],
        mockType: "regex",
      },
      {
        name: "Validation Message",
        desc: "A custom error string shown underneath the input field when any validation rule is violated.",
        types: ["All Types"],
        mockType: "message",
      },
    ],
  },
  {
    ...
    items: [
      {
        name: "Tooltip",
        desc: "A hoverable info circle icon rendering markdown-compatible examples or complex instructions.",
        types: ["All Types"],
        mockType: "tooltip",
      },
      {
        name: "Help Text",
        desc: "Static small text displayed under the input element to provide immediate visual guidance.",
        types: ["All Types"],
        mockType: "help",
      },
      {
        name: "Visibility",
        desc: "Conditional logical statements triggering show/hide states of fields based on preceding field values.",
        types: ["All Types"],
        mockType: "visibility",
      },
    ],
    ...
    items: [
      {
        ...
        desc: "A collection of string values configuring selection menus. Can be loaded dynamically or statically.",
        types: ["Dropdown"],
        mockType: "options",
      },
      {
        name: "Multi Select",
        desc: "Enables a multi-select mode in dropdown fields, converting choices into tags or checklists.",
        types: ["Dropdown"],
        mockType: "multi",
      },
      {
        name: "Nested Fields",
        desc: "Defines the structural blueprint (type, label, and validation) of child fields inside groups.",
        types: ["Repeatable Group"],
        mockType: "nested",
      },
      {
        name: "Repeatable Group",
        desc: "Enables repeating a block of inputs dynamically, allowing lists of nested records to be added.",
        types: ["Repeatable Group"],
        mockType: "repeat",
      },
    ],
...
const renderMockUI = (type: string) => {
  switch (type) {
    case "label":
      return (
        <div className="space-y-1">
          ...
          <input
            key="mock-input-label"
            disabled
            className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-205 dark:border-gray-755 rounded-lg px-3 py-2 text-sm text-gray-400 cursor-not-allowed"
            value="Interactive feedback goes here..."
            readOnly
          />
        </div>
      );
    case "desc":
      return (
        <div className="space-y-1">
          <label className="text-xs font-bold text-gray-955 dark:text-white uppercase tracking-wider block">
            Full Name
          </label>
          ...
          <input
            key="mock-input-desc"
            disabled
            className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-202 dark:border-gray-702 rounded-lg px-3 py-2 text-sm text-gray-400 cursor-not-allowed"
    ...
    case "placeholder":
      return (
        <div className="space-y-1">
          ...
          <input
            key="mock-input-placeholder"
            disabled
            className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-400 placeholder:text-gray-400 dark:placeholder:text-gray-500 cursor-not-allowed"
    ...
    case "default":
      return (
        <div className="space-y-1">
          ...
          <input
            key="mock-input-default"
            disabled
            className="w-full bg-white dark:bg-gray-800 border border-blue-500 rounded-lg px-3 py-2 text-sm text-blue-600 font-medium cursor-not-allowed"
            value="United States (Pre-loaded)"
            readOnly
          />
        </div>
      );
      ...
      return (
        <div className="space-y-2">
          <div className="bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/40 p-2.5 rounded-lg text-xs flex justify-between items-center text-blue-700 dark:text-blue-300">
            ...
            <span className="bg-blue-100 dark:bg-blue-900/40 px-2 py-0.5 rounded text-[10px]">
              Rank: 1
            </span>
          </div>
          <div className="bg-purple-50/50 dark:bg-purple-900/10 border border-purple-100 dark:border-purple-900/40 p-2.5 rounded-lg text-xs flex justify-between items-center text-purple-700 dark:purple-300">
            ...
            <span className="bg-purple-100 dark:bg-purple-900/40 px-2 py-0.5 rounded text-[10px]">
              Rank: 2
            </span>
          </div>
        </div>
    ...
    case "readonly":
      return (
        <div className="space-y-1">
          <label className="text-xs font-bold text-gray-955 dark:text-white uppercase tracking-wider block">
            Unique Database Token
          </label>
          <div className="relative">
            <input
              key="mock-input-readonly"
              disabled
              className="w-full bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg px-3 py-2 text-sm text-gray-400 cursor-not-allowed select-all"
              value="token_987654321_x2_db"
              readOnly
            />
            <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[9px] font-bold text-gray-405 bg-gray-200 dark:bg-gray-800 px-1.5 py-0.5 rounded uppercase">
              Locked
            </span>
          </div>
        </div>
    ...
    case "required":
      return (
        <div className="space-y-1">
          ...
          <input
            key="mock-input-required"
            disabled
            className="w-full bg-white dark:bg-gray-800 border border-red-300 dark:border-red-800/80 rounded-lg px-3 py-2 text-sm text-gray-400 cursor-not-allowed"
            placeholder="Value is required..."
            readOnly
          />
          <span className="text-[10px] text-red-500 font-semibold block">
            ⚠️ Input required before completing row.
          </span>
        </div>
      );
    case "length":
      return (
        <div className="space-y-1">
          <div className="flex justify-between items-center">
            ...
            <span className="text-[9px] text-amber-505 font-bold bg-amber-50 dark:bg-amber-955/20 px-1.5 py-0.5 rounded">
              Min 10 / Max 200
            </span>
          </div>
          <textarea
            key="mock-input-length"
            disabled
            className="w-full bg-white dark:bg-gray-800 border border-amber-300 dark:border-amber-900/60 rounded-lg px-3 py-2 text-xs text-gray-500 h-16 cursor-not-allowed resize-none"
            value="Too short"
            readOnly
          />
          <div className="flex justify-between text-[10px]">
            <span className="text-amber-505 font-semibold">
              ⚠️ Bio must have at least 10 characters.
            </span>
      ...
      return (
        <div className="space-y-1">
          <div className="flex justify-between items-center">
            ...
            <span className="text-[9px] text-purple-500 font-bold bg-purple-50 dark:bg-purple-950/20 px-1.5 py-0.5 rounded">
              Range: 0 - 100
            </span>
          </div>
          <input
            key="mock-input-value"
            disabled
            className="w-full bg-white dark:bg-gray-800 border border-purple-400 rounded-lg px-3 py-2 text-sm text-purple-700 font-bold cursor-not-allowed"
            value="125"
            readOnly
          />
          <span className="text-[10px] text-purple-505 font-semibold block">
            ⚠️ Maximum allowed value is 100.
          </span>
        </div>
      );
    case "regex":
      return (
        <div className="space-y-1">
          <div className="flex justify-between items-center">
            ...
            <span className="text-[9px] text-indigo-505 font-mono bg-indigo-50 dark:bg-indigo-955/20 px-1.5 py-0.5 rounded">
              Pattern: ^\d{5}$
            </span>
          </div>
          <input
            key="mock-input-regex"
            disabled
            className="w-full bg-white dark:bg-gray-800 border border-red-300 dark:border-red-800/80 rounded-lg px-3 py-2 text-sm text-gray-550 cursor-not-allowed"
            value="9021-A"
            readOnly
          />
          <span className="text-[10px] text-red-500 font-semibold block">
            ⚠️ ZIP code must match numeric format (#####).
          </span>
        </div>
      );
    case "message":
      return (
        <div className="space-y-1">
          ...
          <input
            key="mock-input-message"
            disabled
            className="w-full bg-white dark:bg-gray-800 border border-red-500 rounded-lg px-3 py-2 text-sm text-gray-500 cursor-not-allowed"
            value="invalid_id"
            readOnly
          />
          <span className="text-[10px] text-red-550 font-bold block">
            ❌ Custom Validation Error: "Please input exactly 8 digits."
          </span>
        </div>
      );
    case "tooltip":
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-1">
            ...
            <div className="relative group/tool">
              <span className="w-4 h-4 rounded-full bg-blue-50 dark:bg-blue-955 text-blue-600 dark:text-blue-400 text-[10px] font-bold flex items-center justify-center cursor-help border border-blue-200 dark:border-blue-800">
                ?
              </span>
              <div className="absolute left-6 -top-2 w-48 bg-gray-900 dark:bg-gray-955 text-white text-[9px] p-2 rounded-lg shadow-xl opacity-90 leading-normal z-50">
                <strong>Example:</strong> Formatted as XX-XXXXXXX. Look at Box
                4a on your invoice form.
              </div>
            </div>
          </div>
          <input
            key="mock-input-tooltip"
            disabled
            className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-400 cursor-not-allowed"
            placeholder="Hover question mark for details..."
            readOnly
          />
        </div>
      );
      ...
      return (
        <div className="space-y-1">
          ...
          <input
            key="mock-input-help"
            disabled
            className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-750 rounded-lg px-3 py-2 text-sm text-gray-400 cursor-not-allowed"
            placeholder="Enter key..."
            readOnly
          />
          <p className="text-[10px] text-gray-400 leading-snug">
            Note: Keys are case-sensitive and expire every 24 hours.
          </p>
        </div>
      );
    case "visibility":
      return (
        <div className="space-y-3">
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 dark:text-gray-500 uppercase tracking-wider block">
              Has Vehicle?
            </label>
            <div className="flex gap-2">
              <span className="px-3 py-1 bg-blue-500 text-white text-xs font-bold rounded-lg cursor-pointer">
                Yes
              </span>
              <span className="px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-400 text-xs font-bold rounded-lg cursor-pointer">
                No
              </span>
            </div>
          </div>
          <div className="space-y-1 border-l-2 border-blue-500 pl-3 animate-fade-in">
            ...
            <input
              key="mock-input-visibility"
              disabled
              className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-500 cursor-not-allowed"
    ...
    case "options":
      return (
        <div className="space-y-1">
          <label className="text-xs font-bold text-gray-950 dark:text-white uppercase tracking-wider block">
            Select Priority
          </label>
          <div className="border border-gray-205 dark:border-gray-705 bg-white dark:bg-gray-850 rounded-lg p-1.5 space-y-1">
            {[
              ...
              <div
                key={i}
                className="px-2 py-1 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded text-xs text-gray-700 dark:text-gray-300 font-medium flex items-center gap-1.5 cursor-pointer"
              >
        ...
        <div className="space-y-1">
          ...
          <div className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-2 flex flex-wrap gap-1.5 min-h-[40px]">
            {["React", "NestJS", "MongoDB"].map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 text-xs font-bold rounded-md border border-blue-100 dark:border-blue-900/40"
              >
                {tag}
                <span className="text-[9px] text-blue-400 font-bold hover:text-blue-600 cursor-pointer">
                  ×
                </span>
              </span>
            ))}
    ...
    case "nested":
      return (
        <div className="bg-gray-50 dark:bg-gray-900/30 border border-dashed border-gray-200 dark:border-gray-800 rounded-xl p-3 space-y-2">
          <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase block">
            Blueprinted Child Fields
          </span>
      ...
      return (
        <div className="space-y-2">
          <div className="border border-gray-200 dark:border-gray-700 p-2.5 rounded-xl bg-white dark:bg-gray-800 text-xs relative">
            ...
            <span className="absolute right-2.5 top-2.5 text-[9px] text-red-500 font-bold hover:underline cursor-pointer">
              Delete
            </span>
          </div>
          <button className="w-full py-1.5 border border-dashed border-gray-300 dark:border-gray-700 rounded-lg text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-50/20 dark:bg-blue-950/10 hover:bg-blue-50/40 transition-colors">
            + Add Item
          </button>
        </div>
      );
...
export default function UiUxDesign({
  ...
  const [activeFieldTab, setActiveFieldTab] = useState<
    "text" | "number" | "dropdown" | "group"
  >("text");
  const [activeConfigCat, setActiveConfigCat] = useState<string>(
  ...
  return (
    <section
      id="ui-ux-design"
      data-section-id="ui-ux-design"
      className="scroll-mt-28"
    >
      <ZoomBlock delay={0}>
        <div className="flex items-center gap-4 mb-2">
          <span
            className="text-[10px] font-bold tracking-[0.14em] uppercase px-2 py-1 rounded"
            style={{
              background: "#EEF1F8",
              color: INDIGO,
          ...
          <div
            ...
            style={{
              background: "linear-gradient(90deg, #D0CBBE, transparent)",
            }}
          />
        </div>
        <h1
        ...
        <p
          ...
          style={{ color: "#5B6478", maxWidth: "62ch" }}
        >
          High-level architecture, system components, database design, API flow,
          module breakdown, and data flow diagrams.
        </p>
      </ZoomBlock>
      ...
      <StaggerContent className="space-y-6" baseDelay={0.3}>
        ...
        <H2
          ...
          data-subsection-id="design-architecture"
          num="4.1"
          color="blue"
        >
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300 leading-relaxed mb-6 transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
          The platform follows a layered architecture that separates concerns
          across the client, API, service, data, and infrastructure tiers for
        ...
        <div className="relative flex flex-col items-center">
          ...
          {layers.map((layer, i) => (
            <div key={i} className="relative w-full mb-3 last:mb-0">
              <div
                className={`relative bg-gradient-to-r ${layer.gradient} border ${layer.border} rounded-xl p-5 shadow-sm`}
              >
                <div className="flex items-center gap-3">
                  <span
                    className={`inline-flex items-center gap-1.5 px-3 py-1.5 ${layer.pill} text-sm font-bold rounded-full flex-shrink-0`}
                  >
                  ...
                  <div className="flex flex-wrap gap-1.5 flex-1">
                    {layer.chips.map((chip, j) => (
                      <span
                        key={j}
                        className="inline-flex px-2 py-0.5 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 text-xs font-medium rounded-md border border-gray-200/70 dark:border-gray-700 shadow-sm"
                      >
                        {chip}
                      </span>
                    ))}
                  </div>
                  <p className="text-sm font-bold text-gray-400 dark:text-gray-500 text-center mb-8">
        ...
        <H2
          ...
          data-subsection-id="design-components"
          num="4.2"
          color="purple"
        >
          System Components
        ...
        <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-6 text-lg transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
          The platform is composed of modular components organised by
          architectural tier, each with a clearly defined responsibility and
          technology stack.
        </p>
        ...
        {[
            ...
            items: [
              {
                ...
                icon: Monitor,
                desc: "Server-rendered and client-side UI components",
                tech: "Next.js 15 + React 19",
              },
              {
                name: "UI Component Library",
                icon: LayoutDashboard,
                desc: "Reusable Tailwind-based design system",
                tech: "Tailwind CSS 4 + shadcn/ui",
              },
              {
                name: "State & Auth Layer",
                icon: Lock,
                desc: "Client-side auth context and API integration",
            ...
            items: [
              {
                ...
                icon: Server,
                desc: "Modular REST API with Swagger documentation",
                tech: "NestJS 11",
              },
              {
                name: "Auth Service",
                icon: Key,
                desc: "JWT token issuance and Google OAuth integration",
                tech: "JWT + Passport",
              },
              {
                name: "Business Logic Modules",
                icon: Cpu,
                desc: "Domain services for all platform operations",
                tech: "NestJS Modules",
              },
            ],
          },
            ...
            items: [
              {
                ...
                icon: Database,
                desc: "Document-oriented data store for all entities",
                tech: "MongoDB 8",
              },
              {
                name: "Mongoose ODM",
                icon: FileText,
                desc: "Schema modeling and validation layer",
                tech: "Mongoose 8",
              },
            ],
          },
            ...
            items: [
              {
                ...
                icon: HardDrive,
                desc: "Containerised services for consistent deployment",
                tech: "Docker",
              },
              {
                name: "Docker Compose",
                icon: Package,
                desc: "Multi-container orchestration for all tiers",
                tech: "Docker Compose",
              },
            ],
          },
          ...
          const groupDarkText: Record<string, string> = {
            blue: "dark:text-blue-400",
            purple: "dark:text-purple-400",
            green: "dark:text-green-400",
            amber: "dark:text-amber-400",
          };
          const groupDarkBg: Record<string, string> = {
            blue: "dark:bg-blue-900/50",
            purple: "dark:bg-purple-900/50",
            green: "dark:bg-green-900/50",
            amber: "dark:bg-amber-900/50",
          };
          const groupDarkText3: Record<string, string> = {
          ...
          return (
            <div key={group.tier} className="mb-6 last:mb-0">
              <div className="flex items-center gap-2 mb-3">
                <group.icon
                  className={`h-4 w-4 ${c.icon} ${groupDarkText[group.color]}`}
                />
                <span
                  className={`text-sm font-bold ${c.text} ${groupDarkText[group.color]} uppercase tracking-wider`}
                >
                  {group.tier}
                </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {group.items.map((item, j) => (
                  <div
                    key={j}
                    className="bg-white dark:bg-gray-800 border border-gray-200/70 dark:border-gray-700 rounded-xl p-5 shadow-sm transition-transform duration-300 ease-in-out hover:scale-[1.02] hover:shadow-lg cursor-default transform-gpu overflow-hidden"
                  >
                    <div className="flex items-center gap-2.5 mb-2">
                      <div
                        className={`w-8 h-8 rounded-lg ${c.bg} ${groupDarkBg[group.color]} flex items-center justify-center`}
                      >
                        <item.icon
                          className={`h-4 w-4 ${c.icon} ${groupDarkText[group.color]}`}
                        />
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed mb-2 transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
                      {item.desc}
                    </p>
                    <span
                      className={`inline-flex px-2 py-0.5 ${c.bg} ${groupDarkBg[group.color]} ${c.text} ${groupDarkText3[group.color]} text-xs font-medium rounded`}
                    >
                      {item.tech}
                    </span>
                  </div>
        ...
        <H2
          id="design-database"
          data-subsection-id="design-database"
          num="4.3"
          color="amber"
        >
          Database Design
        ...
        <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-6 text-lg transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
          The platform uses MongoDB as its primary data store, with Mongoose
          schemas defining the structure and validation for each collection. The
          document model is designed around the annotation workflow to minimise
          complex joins and keep read performance predictable. The following
          Entity Relationship Diagram (ERD) illustrates the relationships
          between the core collections used throughout the platform.
        </p>

        <div className="flex flex-col items-center gap-3 min-w-[600px]">
          <div
            className={`rounded-xl border-2 border-blue-200 dark:border-blue-800 bg-blue-50/50 dark:bg-blue-900/30 p-3 w-48 text-center shadow-sm`}
          >
          ...
          <div className="flex items-center gap-4">
            <div
              className={`rounded-xl border-2 border-purple-200 dark:border-purple-800 bg-purple-50/50 dark:bg-purple-900/30 p-3 w-48 text-center shadow-sm`}
            >
            ...
            <div
              className={`rounded-xl border-2 border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-900/30 p-3 w-48 text-center shadow-sm`}
            >
          ...
          <div className="flex items-center gap-4">
            ...
            <ChevronRight className="h-4 w-4 text-gray-300" />
            <div
              className={`rounded-xl border-2 border-green-200 dark:border-green-800 bg-green-50/50 dark:bg-green-900/30 p-3 w-48 text-center shadow-sm`}
            >
          ...
          <div className="flex items-center gap-4">
            ...
            <ChevronRight className="h-4 w-4 text-gray-300" />
            <div
              className={`rounded-xl border-2 border-indigo-200 dark:border-indigo-800 bg-indigo-50/50 dark:bg-indigo-900/30 p-3 w-48 text-center shadow-sm`}
            >
          ...
          <div className="flex items-center gap-4">
            <div
              className={`rounded-xl border-2 border-emerald-200 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-900/30 p-3 w-48 text-center shadow-sm`}
            >
            ...
            <ChevronRight className="h-4 w-4 text-gray-300" />
            <div
              className={`rounded-xl border-2 border-rose-200 dark:border-rose-800 bg-rose-50/50 dark:bg-rose-900/30 p-3 w-48 text-center shadow-sm`}
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {dbCollections.map((col, i) => {
            ...
            return (
              <div
                key={i}
                className="bg-white border rounded-2xl shadow-[0_1px_3px_rgba(11,29,51,0.06)] hover:shadow-[0_8px_24px_-12px_rgba(11,29,51,0.18)] transition-shadow duration-300"
                style={{ borderColor: "var(--color-border)" }}
              >
                <div
                  className={`px-3 py-2 ${c.bg} dark:bg-opacity-40 border-b ${c.border} dark:border-opacity-40`}
                >
                ...
                <div className="px-3 py-2">
                  <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
                    {col.purpose}
                  </p>
                </div>
              </div>
        ...
        {/* 4.3.1 */}
        <H2
          id="design-field-schema"
          data-subsection-id="design-field-schema"
          num="4.3.1"
          color="indigo"
        >
          Field Type Data Schema
        ...
        <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-6 text-lg transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
          Every annotation field in DataAnnotate is modeled dynamically and
          stored within MongoDB as a sub-document. Select a field type below to
          view its schema details.
        </p>
          ...
          {[
            ...
            return (
              <button
                ...
                onClick={() =>
                  setActiveFieldTab(
                    tab.id as "number" | "text" | "group" | "dropdown",
                  )
                }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-7 space-y-4">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                  Stores free-form text entered by the annotator, such as entity
                  names, descriptions, or short transcriptions.
                </p>
                <div className="space-y-3 pt-2">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {[
                        ...
                        <span
                          key={v}
                          className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-green-50 dark:bg-green-950/20 text-green-700 dark:text-green-400 text-xs font-semibold rounded-md border border-green-200/50 dark:border-green-900/30"
                        >
                          ✓ {v}
                        </span>
                      ))}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                    <div>
                      ...
                      <code className="text-xs font-mono bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded text-indigo-700 dark:text-indigo-400">
                        "John Doe"
                      </code>
                    </div>
                    <div>
                      ...
                      <span className="text-xs text-gray-600 dark:text-gray-400">
                        Single line input / multi-line text area
                      </span>
                    </div>
                  </div>
                  <div className="pt-1">
                    ...
                    <span className="text-xs text-gray-650 dark:text-gray-350">
                      Stored as a BSON String inside the parent row's annotation
                      document.
                    </span>
                  </div>
                </div>
          ...
          {activeFieldTab === "number" && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-7 space-y-4">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                  Stores numeric values (either integers or decimals) such as
                  ages, quantities, confidence scores, or measurements.
                </p>
                <div className="space-y-3 pt-2">
                  <div>
                    <span className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider block mb-1">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                    ...
                    <div>
                      ...
                      <span className="text-xs text-gray-600 dark:text-gray-400">
                        Spinner number input box with controls
                      </span>
                    </div>
                  </div>
                  <div className="pt-1">
                    ...
                    <span className="text-xs text-gray-650 dark:text-gray-350">
                      Stored as a BSON Double or Int32 property within the
                      parent document.
                    </span>
                  </div>
                </div>
          ...
          {activeFieldTab === "dropdown" && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-7 space-y-4">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                  Provides a predefined set of options for annotators to select
                  a single value or multiple values (classification tags).
                </p>
                <div className="space-y-3 pt-2">
                  <div>
                    <span className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider block mb-1">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                    ...
                    <div>
                      ...
                      <span className="text-xs text-gray-600 dark:text-gray-400">
                        Searchable dropdown select menu
                      </span>
                    </div>
                  </div>
                  <div className="pt-1">
                    ...
                    <span className="text-xs text-gray-650 dark:text-gray-350">
                      Stored as a BSON String (single select) or Array of BSON
                      Strings (multi-select).
                    </span>
                  </div>
                </div>
              </div>
          ...
          {activeFieldTab === "group" && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-7 space-y-4">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                  Groups multiple related fields (nested fields) together. It
                  can repeat multiple times to capture structured list data.
                </p>
                <div className="space-y-3 pt-2">
                  <div>
                    <span className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider block mb-1">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                    ...
                    <div>
                      ...
                      <span className="text-xs text-gray-600 dark:text-gray-400">
                        Visual block container with dynamic "Add Item" button
                      </span>
                    </div>
                  </div>
                  <div className="pt-1">
                    ...
                    <span className="text-xs text-gray-650 dark:text-gray-350 leading-relaxed">
                      Stores child field values as an array of sub-documents
                      representing repetition blocks.
                    </span>
                  </div>
                </div>
              </div>
  ...
  "fields": [
    ...
    { "type": "number", "label": "Pincode" }
        ...
        <H2
          id="design-field-config"
          data-subsection-id="design-field-config"
          num="4.3.2"
          color="amber"
        >
        ...
        <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-6 text-lg transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
          Every configurable property available while creating annotation fields
          is detailed in this interactive configuration sandbox. Select a
          category and property below to preview its live UI demo.
        </p>
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-sm mb-8 space-y-6">
          ...
          <div className="flex flex-wrap gap-2 p-2.5 bg-gray-50 dark:bg-gray-900/40 rounded-xl border border-gray-200/50 dark:border-gray-800/80">
            {(() => {
              const activeCat = configCategories.find(
                (c) => c.name === activeConfigCat,
              );
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {activeItem.types.map((t: string) => (
                        <span
                          key={t}
                          className="inline-flex px-2.5 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs font-bold rounded-md border border-gray-200/50 dark:border-gray-700/50 animate-fade-in"
                        >
                          {t}
                        </span>
                      ))}
        ...
        {/* 4.3.3 */}
        <H2
          id="design-field-validation"
          data-subsection-id="design-field-validation"
          num="4.3.3"
          color="green"
        >
        ...
        <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-6 text-lg transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
          Every field configuration follows a strict lifecycle. Data is
          validated on entry and before saving to secure the integrity of the
          target datasets.
        </p>
        ...
        <div className="relative pl-6 sm:pl-10 space-y-6 mb-8">
          ...
          {[
            {
              step: "1",
              title: "Administrator Creates Field",
              desc: "Admin configures field types, constraints (e.g. required, range boundaries, regex), and custom error messages in the field configuration page.",
              badge: "Setup",
            ...
            },
            {
              ...
              desc: "The configuration schema is persisted to MongoDB inside the AnnotationFields collection and locked if annotations have already started.",
              badge: "Database",
              color: "border-indigo-500",
            },
            {
              step: "3",
              title: "Annotator Opens Workbench",
              desc: "The workbench client retrieves active field definitions from the API and dynamically compiles corresponding React input components.",
              badge: "Interface",
              color: "border-violet-500",
            },
            {
              step: "4",
              title: "Input Validation Triggered",
              desc: "As the annotator types, standard event listeners (on change or on blur) execute validation algorithms client-side to assert formatting and rules.",
              badge: "Client Validation",
              color: "border-fuchsia-500",
            },
            {
              step: "5",
              title: "Valid / Invalid Logic Check",
              desc: "If checks pass, saving is allowed. If checks fail, a Validation Error message is displayed in red, and the next/previous navigation buttons are disabled.",
              badge: "Logic Decision",
              color: "border-pink-500",
            },
            {
              step: "6",
              title: "Autosave Persistence",
              desc: "Once the input matches all rules and is marked valid, an asynchronous HTTP PUT request is triggered in the background to update the user's physical clone document.",
              badge: "Autosave",
              color: "border-rose-500",
            },
            {
              step: "7",
              title: "Consensus Engine Processing",
              desc: "The Consensus Engine merges the isolated clones — generating skeleton reviews if annotations are incomplete, never blocking access, and ensuring structural compatibility using the active validation rules. Every field is assigned a lifecycle status: NOT_STARTED, PENDING, PARTIAL, AGREED, CONFLICT, ADMIN_CONFIRMED, or OVERRIDDEN.",
              badge: "Consensus",
            ...
            <div key={i} className="relative flex items-start gap-4">
              ...
              <div className="flex-1 bg-white dark:bg-gray-800 border border-gray-200/75 dark:border-gray-700 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="px-2.5 py-0.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-xs font-bold rounded-full">
                    {item.badge}
                  </span>
                </div>
                <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
        ...
        <p className="text-[11px] font-bold text-gray-400 dark:text-gray-500 mt-5 text-center tracking-wider uppercase mb-12">
          Dynamic Schema Field Validation Workflow
        </p>

        {/* 4.4 */}
        <H2
          id="design-api"
          data-subsection-id="design-api"
          num="4.4"
          color="indigo"
        >
          API Flow
        ...
        <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-6 text-lg transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
          Every API request passes through a consistent pipeline that validates
          identity, authorisation, and payload integrity before reaching the
          service layer.
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
          {apiOutcomes.map((outcome, i) => (
            <div
              key={i}
              className={`${outcome.color} rounded-xl px-4 py-3 border shadow-sm transition-all duration-200 hover:shadow-md cursor-default`}
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm font-bold">{outcome.label}</span>
                <div
                  className={`w-2 h-2 rounded-full ${outcome.color.includes("green") ? "bg-green-500" : outcome.color.includes("orange") ? "bg-orange-500" : "bg-red-500"}`}
                />
              </div>
            ...
            {apiSteps.map((step, i) => (
              <div key={i} className="flex items-center">
                <div className="flex flex-col items-center gap-2 min-w-[120px]">
                  <div
                    className={`w-12 h-12 rounded-xl ${step.color} flex items-center justify-center shadow-sm border border-gray-200/50 dark:border-gray-700/50`}
                  >
                    <step.icon className="h-5 w-5" />
                  </div>
                  <span
                    className={`text-[11px] font-bold ${step.color} px-2 py-0.5 rounded-full whitespace-nowrap`}
                  >
                    {step.label}
                  </span>
                </div>
        ...
        <p className="text-[11px] font-bold text-gray-400 dark:text-gray-500 text-center tracking-wider uppercase mb-8">
              ...
              <div key={i} className="relative flex items-start gap-5">
                ...
                <div className="flex-1 bg-white dark:bg-gray-800 border border-gray-200/70 dark:border-gray-700 rounded-xl p-3.5 shadow-sm transition-all duration-200 hover:shadow-md hover:border-indigo-200 dark:hover:border-indigo-800 cursor-default">
                  <div className="flex items-center gap-2.5 mb-1">
                    <span
                      className={`inline-flex px-2.5 py-0.5 ${item.color} text-[11px] font-bold rounded-full`}
                    >
                      {item.actor}
                    </span>
                  </div>
        ...
        <p className="text-[11px] font-bold text-gray-400 dark:text-gray-500 mt-5 text-center tracking-wider uppercase">
        ...
        <H2
          id="design-modules"
          data-subsection-id="design-modules"
          num="4.5"
          color="emerald"
        >
          Module Breakdown
        ...
        <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-6 text-lg transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
          Platform functionality is organised into four tiers, each grouping
          related modules that together deliver end-to-end business
          capabilities.
        </p>
          ...
          {(() => {
            ...
            return tiers.map((tier, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`relative overflow-hidden rounded-xl border ${c.border} dark:border-opacity-40 bg-white dark:bg-gray-800 shadow-sm transition-all duration-200 hover:shadow-md cursor-default`}
                >
                  <div className={`h-1.5 w-full ${c.bg}`} />
                  <div className="p-4">
                    <div className="flex items-center gap-2 mb-3">
                      <div
                        className={`w-8 h-8 rounded-lg ${c.bg} dark:bg-opacity-40 flex items-center justify-center text-base`}
                      >
                        {tierIcons[tier.label]}
                      </div>
                      <span className={`text-sm font-bold ${c.text}`}>
                        {tier.label}
                      </span>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {tier.chips.map((chip, j) => (
                        <span
                          key={j}
                          className={`inline-flex px-2.5 py-1 ${c.bg} dark:bg-opacity-30 ${c.text} text-xs font-bold rounded-full border ${c.border} dark:border-opacity-50`}
                        >
                          {chip}
                        </span>
                      ))}
            ...
            {moduleDetails.map((mod, i) => {
              ...
              return (
                ...
                >
                  {/* Timeline dot */}
                  <div
                    className="relative z-10 hidden md:flex w-[46px] h-[46px] rounded-full bg-white dark:bg-gray-800 border-2 flex-shrink-0 items-center justify-center shadow-sm transition-all duration-200 group-hover:shadow-md group-hover:scale-105"
                    style={{ borderColor: hex[mod.color] }}
                  >
                    <span
                      className="text-xs font-bold"
                      style={{ color: hex[mod.color] }}
                    >
                      {i + 1}
                    </span>
                  </div>
                  {/* Card */}
                  <div
                    className="flex-1 bg-white dark:bg-gray-800 border border-gray-200/70 dark:border-gray-700 rounded-xl p-5 shadow-sm transition-all duration-200 group-hover:shadow-md cursor-default"
                    style={{ borderLeft: `4px solid ${hex[mod.color]}` }}
                  >
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                      <div className="flex-1">
                        <div className="flex items-center gap-2.5 mb-1">
                          <span
                            ...
                            style={{ backgroundColor: hex[mod.color] }}
                          />
                          <p className="text-base font-bold text-gray-900 dark:text-white">
                            {mod.name}
                          </p>
                      ...
                      <div className="flex flex-wrap gap-1.5 flex-shrink-0">
                        {mod.actions.map((action, j) => (
                          <span
                            key={j}
                            className={`inline-flex px-2.5 py-1 ${c.bg} dark:bg-opacity-30 ${c.text} dark:dark-text-opacity-90 text-xs font-bold rounded-full border ${c.border} dark:border-opacity-50 shadow-sm`}
                          >
                            {action}
                          </span>
                        ))}
        ...
        {/* 4.6 */}
        <H2 id="design-ui" data-subsection-id="design-ui" num="4.6" color="sky">
          UI Flow
        </H2>
        <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-6 text-lg transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
          The platform provides distinct screen flows for administrators and
          annotators, with each role seeing only the interfaces relevant to
          their permissions.
        </p>
          ...
          <div className="relative">
            ...
            <div className="relative flex flex-col items-center">
              ...
              {adminScreens.map((screen, i) => (
                <div
                  key={i}
                  className="relative mb-6 last:mb-0 w-full max-w-xs"
                >
                  {/* Connector dot */}
                  ...
                  {i < adminScreens.length - 1 && (
                    <div className="flex justify-center mt-2">
                      <svg
                        className="w-4 h-4 text-blue-300 dark:text-blue-700"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        strokeWidth={2}
              ...
              {annotatorScreens.map((screen, i) => (
                ...
                >
                  ...
                  {i < annotatorScreens.length - 1 && (
                    <div className="flex justify-center mt-2">
                      <svg
                        className="w-4 h-4 text-amber-300 dark:text-amber-700"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        strokeWidth={2}
        ...
        <H2
          id="design-data"
          data-subsection-id="design-data"
          num="4.7"
          color="teal"
        >
          Data Flow
        ...
        <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-6 text-lg transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
          Data moves through a defined pipeline from raw CSV upload through
          validation, isolated annotation, and consensus generation, producing a
          single authoritative dataset ready for model training. Each stage
          enforces specific business rules that guarantee data integrity and
        ...
        <div className="flex flex-col items-center gap-1.5 mb-8">
          {dataFlowStages.map((stage, i) => (
            <div key={i} className="flex flex-col items-center">
              <div
                className={`flex items-center gap-2 px-4 py-2.5 ${stage.color} text-sm font-bold rounded-xl border-2 shadow-sm min-w-[180px] justify-center`}
              >
                <stage.icon className="h-4 w-4" />
                {stage.label}
              </div>
              {i < dataFlowStages.length - 1 && (
                <ChevronRight className="h-4 w-4 text-gray-300 -rotate-90 my-0.5" />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center justify-center gap-3">
          {lifecycleStates.map((state, i) => (
            <div key={i} className="flex items-center gap-3">
              <div
                className={`px-4 py-2 ${state.color} text-sm font-bold rounded-full border-2 shadow-sm`}
              >
                {state.state}
              </div>
              {i < lifecycleStates.length - 1 && (
                <div className="flex flex-col items-center">
                  ...
                  {state.transition && (
                    <span className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 whitespace-nowrap">
                      {state.transition}
                    </span>
                  )}
                </div>
        ...
        {lifecycleStates[2].transition && (
          <div className="flex items-center justify-center gap-1 mt-3 mb-2">
            ...
            <span className="text-xs text-gray-400 dark:text-gray-500">
              Login reactivates Active state
            </span>
          </div>
        )}
        ...
        {/* 4.8 */}
        <H2
          id="design-product-docs"
          data-subsection-id="design-product-docs"
          num="4.8"
          color="purple"
        >
        ...
        <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-6 text-lg transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
          DataAnnotate includes integrated, context-sensitive in-product
          documentation covering the entire annotation workflow, user settings,
            ...
            {showAllDocs ? (
              ...
              <div className="p-6 space-y-8 w-full">
                {docPages.map((page, idx) => (
                  <div
                    key={idx}
                    className="border-b border-gray-150 dark:border-gray-800 pb-8 last:border-b-0 last:pb-0"
                  >
                    <div className="flex items-center gap-2 mb-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <p className="text-sm text-gray-655 dark:text-gray-305 leading-relaxed mt-1">
                          {page.purpose}
                        </p>
                      </div>
                      <div>
                        ...
                        <span className="inline-flex px-2.5 py-1 bg-purple-50 dark:bg-purple-950/20 text-purple-700 dark:text-purple-400 text-xs font-bold rounded-full mt-1 border border-purple-200/50 dark:border-purple-800/30">
                          {page.reader}
                        </span>
                      </div>
                    </div>
                    <div className="border-t border-gray-100 dark:border-gray-750/60 pt-4 mb-4">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {page.features.map((feature, fIdx) => (
                          <span
                            key={fIdx}
                            className="inline-flex px-2.5 py-0.5 bg-gray-105 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs font-medium rounded-md"
                          >
                            {feature}
                          </span>
                        ))}
              ...
              <div className="flex flex-col lg:flex-row min-h-[500px] w-full">
                ...
                <div className="w-full lg:w-72 border-r border-gray-250 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30 p-5 space-y-5 flex-shrink-0">
                  <div className="space-y-1.5">
                    <span className="text-[10px] font-bold text-gray-455 dark:text-gray-500 uppercase tracking-wider block px-1">
                      Categories
                    </span>
                    <div className="grid grid-cols-2 lg:grid-cols-1 gap-1">
                      {docCategories.map((cat) => {
                        ...
                        const colorClasses = {
                          ...
                          purple:
                            "bg-purple-50/60 text-purple-700 border-purple-200/50 dark:bg-purple-900/40 dark:text-purple-300 dark:border-purple-800/40",
                          green:
                            "bg-green-50/60 text-green-700 border-green-200/50 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800/40",
                          amber:
                            "bg-amber-50/60 text-amber-700 border-amber-200/50 dark:bg-amber-900/40 dark:text-amber-300 dark:border-amber-800/40",
                  ...
                  <div className="space-y-1.5 pt-3 border-t border-gray-200 dark:border-gray-850">
                    ...
                    <div className="space-y-1">
                      {(() => {
                        const activeCat = docCategories.find(
                          (c) => c.name === activeDocCat,
                        );
                        if (!activeCat) return null;
                            ...
                            <button
                              ...
                              className={`w-full text-left px-3 py-2 rounded-lg text-xs font-semibold transition-all flex items-center justify-between ${
                                isSelected
                                  ...
                                  : "text-gray-600 dark:text-gray-500 hover:bg-white/50 dark:hover:bg-gray-850/50"
                              }`}
                            >
                              <span>{page.title}</span>
                              <ChevronRight
                                className={`h-3 w-3 transition-transform ${isSelected ? "translate-x-0.5 text-blue-500" : "text-gray-400 opacity-60"}`}
                              />
                            </button>
                          );
                        });
                    ...
                    return (
                      <div className="space-y-6 animate-fade-in">
                        ...
                        <div className="flex items-center gap-1.5 mb-5 border-b border-gray-100 dark:border-gray-750 pb-4">
                          {["Overview", "Roadmap", "Quality & Hazards"].map(
                            (stepTitle, stepIdx) => {
                          ...
                          {docViewerSlide === 0 && (
                            <div className="space-y-4 animate-fade-in">
                              <div className="space-y-1 bg-white/30 dark:bg-white/5 p-4 rounded-xl border border-white/20">
                                ...
                                <p className="text-sm text-gray-750 dark:text-gray-200 leading-relaxed font-semibold">
                                  {page.purpose}
                                </p>
                              </div>

                              <div className="space-y-2">
                                <span className="text-[10px] font-bold text-gray-500 dark:text-gray-500 uppercase tracking-wider block">
                                  Features Covered
                                </span>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                  {page.features.map((feature, fIdx) => (
                                    <div
                                      key={fIdx}
                                      className="flex items-center gap-2 p-2.5 bg-white/70 dark:bg-gray-800/70 text-gray-700 dark:text-gray-300 text-xs font-semibold rounded-xl border border-gray-200/50 dark:border-gray-700/50 shadow-sm"
                                    >
                            ...
                            <div className="space-y-3 animate-fade-in">
                              ...
                              <div className="space-y-2.5">
                                {page.steps.map((step, sIdx) => (
                                  <div
                                    key={sIdx}
                                    className="flex gap-3 items-start p-3 bg-white/60 dark:bg-gray-800/60 rounded-xl border border-gray-200/50 dark:border-gray-700/50 shadow-sm transition-transform duration-300 hover:scale-[1.01]"
                                  >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in">
                              ...
                              <div className="space-y-4">
                                ...
                                <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                                  <span className="text-[10px] font-bold text-gray-455 dark:text-gray-550 uppercase tracking-wider block mb-1.5">
                                    Related Pages
                                  </span>
                                        ...
                                        <button
                                          ...
                                          onClick={() => {
                                            if (pageIndex !== -1) {
                                              ...
                                              const cat = docCategories.find(
                                                (c) =>
                                                  c.pageIndices.includes(
                                                    pageIndex,
                                                  ),
                                              );
                                              if (cat) {
        ...
        <H2
          id="design-html-docs"
          data-subsection-id="design-html-docs"
          num="4.8.1"
          color="gray"
        >
          HTML Documentation
        ...
        <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-6 text-lg transition-transform duration-200 ease-in-out hover:scale-[1.02] transform-gpu cursor-default">
          DataAnnotate features an integrated HTML documentation module
          available directly in the user interface. This enables developers and
          stakeholders to access high-quality reference guides without leaving
        ...
        <div className="bg-gray-50/50 dark:bg-gray-900/30 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 shadow-sm mb-6">
          <p className="text-sm font-bold text-gray-900 dark:text-white mb-3">
            Key Features of the Integrated Documentation Module:
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <div
                key={i}
                className="flex items-start gap-2.5 p-3 bg-white dark:bg-gray-800 border border-gray-200/50 dark:border-gray-700 rounded-xl shadow-sm"
              >
                <span className="w-2.5 h-2.5 rounded-full bg-blue-500 mt-1.5 flex-shrink-0" />
                <span className="text-sm text-gray-650 dark:text-gray-350 leading-relaxed">
                  {feature}
                </span>
              </div>
            ))}
        ...
        <div className="overflow-x-auto pb-4 mb-6">
          <div className="flex items-center min-w-max justify-center py-4">
            {[
              {
                label: "Browser",
                desc: "Renders UI & triggers search events",
                color:
                  "bg-blue-50 text-blue-700 border-blue-200/60 dark:bg-blue-900/30 dark:text-blue-300",
              },
              {
                label: "HTML Docs",
                desc: "React wrapper holding view states",
                color:
                  "bg-purple-50 text-purple-700 border-purple-200/60 dark:bg-purple-900/30 dark:text-purple-300",
              },
              {
                label: "Markdown Renderer",
                desc: "Converts Markdown assets to HTML elements",
                color:
                  "bg-amber-50 text-amber-700 border-amber-200/60 dark:bg-amber-900/30 dark:text-amber-300",
              },
              {
                label: "React Components",
                desc: "Applies theme classes and layouts",
                color:
                  "bg-green-50 text-green-700 border-green-200/60 dark:bg-green-900/30 dark:text-green-300",
              },
              {
                label: "Search Index",
                desc: "Extracts and stores text nodes for querying",
                color:
                  "bg-indigo-50 text-indigo-700 border-indigo-200/60 dark:bg-indigo-900/30 dark:text-indigo-300",
              },
              {
                label: "Documentation Pages",
                desc: "Source guides formatted as code models",
                color:
                  "bg-emerald-50 text-emerald-700 border-emerald-200/60 dark:bg-emerald-900/30 dark:text-emerald-300",
              },
            ].map((step, i, arr) => (
              <div key={i} className="flex items-center">
                <div className="flex flex-col items-center gap-2 max-w-[150px] text-center">
                  <div
                    className={`px-4 py-2.5 rounded-xl border ${step.color} shadow-sm font-bold text-sm min-h-[50px] flex items-center justify-center`}
                  >
                    {step.label}
                  </div>
                  <span className="text-[10px] text-gray-500 dark:text-gray-400 px-1 leading-snug">
                    {step.desc}
                  </span>
                </div>
                {i < arr.length - 1 && (
